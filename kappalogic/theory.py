...
def or_n_optimal_fold_fires(values, xi):
    """
    【命題35(v0.75): 最適順序(降順)の発火は"上位2つの和"で決まる——
    順序が集約則を MAX↔SUM で補間することの精密化】

    命題23は「逐次 fold は OR を SUM 集約から MAX 集約に劣化させる」ことを
    示したが、命題30・32はその劣化が**順序依存**だと示していた。本命題は
    その機構を、最適順序(L 降順=命題30)について**閉形式**で確定させる:

        **L 降順に畳んだ naive fold は発火する(蹴られた walk が t* を越える)
          ⟺(ほぼ)L_(1) + L_(2) > t*(xi)**

    (L_(1) >= L_(2) は最大2つの対数増分 L(a_k;xi)=2 ln cosh(a_k/xi)。)

    機構(なぜ上位2つか): 降順では最大の L_(1) が先頭に来て reset を受けず
    mu_1 = L_(1)。次に s_2 = mu_1 + L_(2) = L_(1) + L_(2) が、reset を挟まない
    "生"の最大和になる。これが t* を越えれば**ステップ2で即座に crossing**。
    越えなければ mu_2 = G(s_2) < s_2 と縮み、以降の L_k <= L_(2) は蓄積し
    にくい。したがって降順の発火は実質「上位2つの和が t* に届くか」で決まる。

    == 厳密な十分性(偽陽性0)==
    L_(1)+L_(2) > t* なら s_2 = L_(1)+L_(2) > t* が定義上成り立ち、必ず
    crossing する——これは**証明可能な十分条件**(下記検証で偽陽性0件)。

    == 近必要性(実測)==
    見逃す(crossing するのに rule が false)のは、上位2つでは届かず3項以上の
    残差蓄積で越える稀なケースのみ。xi in {0.2,...,0.01}・n<=12 の乱数40000
    試行で:偽陽性0件、偽陰性12件(全crossingの0.030%)、総合一致99.97%。

    == 集約則の補間(命題23の精密化)==
    純粋な蓄積レジーム(どの単独 L_k も t* 未満、かつ Σ L_k > t*)での crossing 率:
        降順 ~90% (SUM則[Σ>t*]をほぼ回復)  >  ランダム ~55%  >  昇順 ~22%
        (MAX則[max L>t*]=誰も越えないので非発火、に留まる)。
    すなわち命題23の「MAX 劣化」は順序に無頓着な場合の話で、**降順に畳めば
    SUM 集約を大きく回復する**。命題30(降順=発火最大)・命題32(順序依存
    発火)の機構的な説明にもなっている(降順の発火バーが「上位2和>t*」と
    最も低い)。

    正直な限界:
    - 「ほぼ」は近必要性(0.03%の偽陰性=3項以上の蓄積)を指す。厳密に
      成り立つのは**十分性の向き**(L_(1)+L_(2)>t* => 発火)のみ。
    - crossing と naive fold の発火(>0.5)の同一視は命題25の近似(0.5境界の
      浮動小数点同着を除き厳密、~99.6%)を継承する。
    - 命題34 FALSE側(Σ L_k <= t* => どの順序でも非発火)と本命題(上位2和
      >t* => 降順発火)で、降順の発火/非発火を上下から挟める。残る不定帯は
      「Σ>t* だが L_(1)+L_(2)<=t*」の多項蓄積領域(全体の~0.03%が実際に
      発火)。

    戻り値: bool(L_(1)+L_(2) > t*(xi))。n=1 のときは L_(1) > t*。
    """
    values = np.asarray(values, dtype=float)
    Ls = np.sort(2 * np.log(np.cosh(values / xi)))[::-1]
    tstar = or_n_kicked_map_unstable_point(xi)
    top = Ls[0] if len(Ls) == 1 else Ls[0] + Ls[1]
    return bool(top > tstar)
