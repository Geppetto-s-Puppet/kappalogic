...
【命題19(v0.37): XORの2変数(a,b)全体での誤分類境界——ついに完成】
命題13(b=0断面)・14(対角線a=b)は、XORの誤分類境界のうち特定の
断面だけを解いたものだった。torch_backend.pyを使って(a,b)平面
全体の危険地図を高速に走査したところ、u=a/xi, v=b/xiの言葉で、
**片方が十分大きいとき、もう片方の境界がlog(u)(またはlog(v))
でスケールする**という、命題8(u+vの和)とも命題12(NORの二重対数)
とも違う、新しい形の境界が見つかった:

    v_boundary(u) ~= (1/2)*ln(4*u / u2(xi))     (uが大きい極限)
    u_boundary(v) ~= (1/2)*ln(4*v / u2(xi))     (対称、vが大きい極限)

    (u2(xi) := xor_zero_cross_section_threshold(xi)、命題13の閾値)

導出: uが大きいとき、NOT(a;xi)~=4*exp(-2u)は極端に小さくなるので、
Q:=AND(NOT(a),b;xi)はほぼ0になり無視できる。するとXOR(a,b)=OR(P,Q)
~=OR(P,0)=NOT(NOT(P);xi)という、命題13と同じ"二重NOT"の構造に
帰着する。ここでP:=AND(a,NOT(b);xi)=reg(a*NOT(b);xi)=
tanh(u*sech^2(v))^2 であり、命題13の閾値u2(xi)(reg(a;xi)=u2相当と
なる点)に対応させると、u*sech^2(v)~=u2(xi)という条件になる。
vが大きい極限でsech^2(v)~=4*exp(-2v)を使うと、
u*4*exp(-2v)~=u2(xi)を解いて上のv_boundary(u)の式が出る。

数値検証: xi=1e-6〜1e-10、u=10〜10000の広い範囲で、実測境界と
予測の差が0.0001以下(小数点以下4桁まで一致)という、非常に高い
精度で確認できた。対称性(uとvを入れ替えても同じ形になること)も
確認済み。

**これで命題11〜15・19を合わせて、6ゲート全部について、少なくとも
1つの完全な2次元的境界(NAND・OR・NORは実質2次元完成、XORは今回
v0.37で2次元完成)が得られたことになる**。XNORについても同様の
手法(命題15のb=0断面を出発点に、片方が大きい極限を取る)が
使えるはずだが、今回はまだ試していない。

正直な評価: この閉形式は「片方の変数が十分大きい」という漸近極限
でのみ厳密に成り立つ(u,vが同程度でどちらも大きくない"中間領域"
では、命題14の対角線の結果のようにまた別の振る舞いになる)。
それでも、TODO.mdで積み残していた「各ゲートの(a,b)平面全体の
完全な閉形式」という課題に、XORについては具体的な答えを出せた。

【命題20(v0.38): XNORの2変数(a,b)境界——6ゲート全部が2次元完成】
命題19(XORの2次元境界)と全く同じ手法をXNORに適用した。
XNOR=NOT(XOR;xi)なので、uが大きい極限でXOR(a,b)がP:=tanh(u*sech^2(v))^2
の"二重NOT"に帰着する(命題19)ことを踏まえると、XNORはPの
**三重NOT**になる——これは命題15(XNORのb=0断面)と全く同じ構造
だったので、命題15の閾値u3(xi):=xnor_zero_cross_section_threshold(xi)
をそのまま使って

    v_boundary(u) ~= (1/2)*ln(4*u / u3(xi))     (uが大きい極限)

という、命題19と同じ形の閉形式が(定数をu2からu3に変えるだけで)
そのまま成り立つことを確認した。

数値検証: xi=1e-4〜1e-10、u=10〜10000の範囲で、実測境界との差が
0.0001以下という、命題19と同等の高精度で一致することを確認した。

**これで6ゲート(AND, OR, NAND, NOR, XOR, XNOR)全部について、
(a,b)平面全体の完全な閉形式(NAND・OR・NORは単一変数への還元や
u+vの対称性から、XOR・XNORは"片方が大きい極限"の解析から)が
出揃ったことになる**。TODO.mdで積み残していた課題が、命題5・
11〜15・19・20を通じて完全に解決した形になる。

正直な評価: 命題19の手法がXNORにもそのまま(定数を1つ変えるだけで)
通用したのは、XOR→XNORが単純に"もう1回NOTを重ねるだけ"という
構造だったから。目新しい導出はなく、命題19の直接の系
(corollary)に近い。それでも、6ゲート全部の2次元的な理解が
完成した、というシリーズ全体の到達点として記録しておく。
"""
import numpy as np
