...
def or_n_tree_fold(values, xi):
    ...
    == v0.75 追記: 集約回復と"順序ロバスト性"の定量化(命題35と接続)==
    純粋な蓄積レジーム(どの単独 L_k も t* 未満、かつ Σ L_k > t*)での SUM 回復率
    (=fused が true のとき実際に発火する率、高いほど良い)を fold 構造×順序で
    測ると(xi=0.1、実測):
        逐次-降順 ~88%  ≈  木-降順 ~86%   >   木-昇順 ~76%   >
        木-ランダム ~61%  >  逐次-ランダム ~44%  >>  逐次-昇順 ~18%
    読み方: (1) **降順に畳めるなら逐次で SUM を最大回復**(命題35、上位2和則)、
    (2) **順序を制御できないなら tree fold が圧倒的にロバスト**——最悪順序での
    SUM 回復が逐次18%に対し木76%。tree fold の"1/7に不一致減少"の正体は、
    隣接ペアの和がブロック内で SUM を先に取る(命題23/24)ことで、悪い順序でも
    残差蓄積が reset で消えにくくなる**順序ロバスト性**だった。
    """
...
def or_n_optimal_fold_fires(values, xi, steps=2):
    """
    【命題35(v0.75): 最適順序(降順)の発火は"最初の数フォールド"で決まる
    ——有限地平の決定則、および順序が集約則を MAX↔SUM で補間することの精密化】

    命題23は「逐次 fold は OR を SUM 集約から MAX 集約に劣化させる」ことを
    示したが、命題30・32はその劣化が**順序依存**だと示していた。本命題は
    その機構を、最適順序(L 降順=命題30)について**閉形式**で確定させる。

    L 降順に畳むと蹴られた walk μ_1=L_(1), μ_k=G(μ_{k-1}+L_(k)) は最大の
    L_(1) を先頭に置く(reset を受けない)ので、crossing は**最初の数ステップ
    に集中する**。実測(純粋蓄積レジーム)では初 crossing の 98〜100% が
    ステップ2、残り 1〜2% がステップ3で、ステップ4以降はほぼ皆無。

    **k段打ち切り則(本関数、`steps=k`): 降順 walk が最初の k フォールド以内で
    t* を越えるか**。これは実 walk の打ち切りなので、**任意の k で厳密な
    十分条件**(True を返せば必ず本当に crossing する=偽陽性0)。k を増やすと
    必要性が単調に改善する:

        k=2 (既定, =「L_(1)+L_(2) > t*」の上位2和則):
            偽陽性0、偽陰性0.023%、総合一致 99.977%
        k=3 (「G(L_(1)+L_(2))+L_(3) > t*」まで見る):
            偽陽性0、偽陰性0.003%、総合一致 99.997%
        k=4: 偽陽性0、偽陰性0.000%、総合一致 100.000%
      (xi∈{0.2,0.1,0.05,0.02,0.01}・n<=12 の乱数30000試行、実測値)

    すなわち「**最適順序の発火は、最大でも上位4つの値だけで決まる有限地平の
    決定**」であり、鎖の残り(小さい L)は結論を変えない。既定の k=2 が最も
    単純な閉形式(上位2和 > t*)で 99.98%、k=4 で実測100%。

    == 機構(なぜ上位2つが主役か)==
    降順先頭は reset を受けず μ_1=L_(1)、次に s_2=L_(1)+L_(2) が reset を挟ま
    ない"生"の最大和。これが t* を越えれば**ステップ2で即 crossing**。
    越えなければ μ_2=G(s_2)<s_2 と縮み、以降の L_k<=L_(2) は蓄積しにくい。

    == 集約則の補間(命題23の精密化)==
    純粋な蓄積レジーム(どの単独 L_k も t* 未満、かつ Σ L_k > t*)での crossing 率:
        降順 ~90% (SUM則[Σ>t*]をほぼ回復)  >  ランダム ~55%  >  昇順 ~22%
        (MAX則[max L>t*]=誰も越えないので非発火、に留まる)。
    命題23の「MAX 劣化」は順序に無頓着な場合の話で、**降順に畳めば SUM 集約を
    大きく回復する**。命題30(降順=発火最大)・命題32(順序依存発火)の機構的
    説明にもなっている(降順の発火バーが最も低い)。

    正直な限界:
    - 厳密に成り立つのは**十分性の向き**(k段以内で越える => 発火)のみ。
      逆(発火 => k段以内で越える)は近必要で、k=4 でも「実測100%」であって
      証明された100%ではない(ステップ5+ crossing は乱数30000試行で0件だが、
      存在しないことの証明ではない)。
    - crossing と naive fold の発火(>0.5)の同一視は命題25の近似(0.5境界の
      浮動小数点同着を除き厳密、~99.6%)を継承する。
    - 命題34 FALSE側(Σ L_k <= t* => どの順序でも非発火)と本命題(k段以内で
      越える => 降順発火)で、降順の発火/非発火を上下から挟める。残る不定帯は
      「Σ>t* だが k段以内で越えない」多項蓄積領域(k=2 で~0.02%、k=4 で実測0%)。

    戻り値: bool(降順 walk が最初の `steps` フォールド以内で t* を越えるか)。
            n < steps のときは在る分だけ回す。既定 steps=2。
    """
    values = np.asarray(values, dtype=float)
    Ls = np.sort(2 * np.log(np.cosh(values / xi)))[::-1]
    tstar = or_n_kicked_map_unstable_point(xi)
    mu = Ls[0]
    if mu > tstar:            # step 1: 単独で t* を越える(命題10 の単独大)
        return True
    for Lk in Ls[1:max(steps, 1)]:
        s = mu + Lk
        if s > tstar:         # step j: s_j = μ_{j-1}+L_(j) が越える
            return True
        mu = or_n_log_kicked_map(s, xi)
    return False
