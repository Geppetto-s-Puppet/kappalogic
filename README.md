...
| `examples/` | 実行できるデモ17本(命題32の順序依存発火、v0.76のSP2線形スケーリングDFT、v0.77の量子測定、v0.78のBCS超伝導、v0.80の電子輸送、v0.81の磁性、v0.82の分岐分類、v0.83の色と乱れ、v0.84のAnderson転移、v0.85の乱れた物質、v0.86の色と伝導、v0.93のThouless窓デモを含む)+ 外付け実験3件(`kappa_cpu/`, `gauge_adder_chain/`, `cuda/`。下記「関連実験」参照) |
...
| 41 | **ξ分解能則——「ξをどれだけ大きく取るか」が局在の診断になる(v0.90)** | v0.87(久保σのη)とv0.89(逆設計のξ)で独立に出た「ξが離散化スケールより小さいと壊れる」を**定量則に格上げ**した。ξ幅の窓は`N_eff≈ξ/Δ`個の準位を平均するので、配位間の相対ゆらぎが`∝ξ^p`となる。**pはスペクトル統計で決まる**: 剛的(準位反発=拡張)なら`Var(n)~ln n`で`p≈−1`以下、Poisson(独立=局在)なら`Var(n)=n`で`p→−0.5`。実測(1D乱れ鎖N=400、300配位)は`⟨r⟩`の低下と**完全に同期**: `W=0.5: ⟨r⟩0.591, p=−1.454` → `W=2: 0.408, −1.111` → `W=12: 0.388, −0.664`。**帰結2つ**: (a)**ξの処方箋が系依存**——相対精度εに必要なξは拡張系`≈Δ/ε`だが**局在系は`≈Δ/ε²`**(Δ=0.01, ε=0.05なら`0.2` vs `4.0`と**20倍差**);(b)**固有値だけで測れる局在診断**——IPRは固有ベクトル、転送行列は別計算が要るが、本量は固有値のアンサンブルのみで得られる。v0.87/v0.89の失敗と命題39の局在物理が1つの根に統一された。**(v0.91で完成)** 純アンサンブルで**較正**したところ理論値に一致: **picket(超剛的) −1.501 / GOE −0.968(理論−1) / Poisson −0.594(理論−0.5)**。そして**ξには両側の境界がある**と判明——下限`ξ≳Δ`(小さすぎると離散化アーティファクト、**機構は明確**)に加え、**上限`ξ/Δ≲10`程度**(大きすぎると指数が壊れる)。**(v0.92で機構を訂正)** 上限を「窓がDOS構造を跨ぐため」と説明していたが**検証したら誤りだった**——DOSが平坦に留まる幅は`ξ/Δ≈54〜146`と破綻点(≈16)より1桁大きく、また局所傾きは単純な飽和でなく中間に肩を作る非単調な振る舞いを示す。**上限は経験則であって機構の説明ではない**と訂正し、代わりに機構を仮定せず**データから安定区間を自動選択する`resolution_exponent_auto`**を追加(局所傾きが一致する最長区間でフィット、無ければ`None`を返す)。選ばれる窓は系ごとに異なる(3D W=6→`(2,4,8,16)`、W=30→`(0.5,1,2,4)`)——正しい窓が系依存であることの直接の証拠。3D Anderson の拡張相(W=6、純GOEのはず)で `ξ/Δ=4..64→−0.527 / 2..16→−0.797 / **1..8→−0.971**` と、**局所窓にして初めてGOE値に一致**。既定を局所窓`(1,2,4,8)`に変更した結果、**1Dでも3Dでも純アンサンブル値を再現**: 1D `W=12→−0.493`(理論−0.5)、3D `W=6→−0.971`(GOE)・`W=30→−0.565`(Poisson)。すなわち **Δ ≪ ξ ≪ E_DOS の窓で使う**。**限界**: 相は同定できるが**転移点W_cは定まらない**(3D L=8ではW=16.5が既にPoisson寄り。v0.87と同じ有限サイズの制約で、W_c精密決定は転送行列へ) |
| 42 | **ξ窓の上限はThoulessスケール——窓の幅が無次元コンダクタンス(v0.93)** | v0.91-0.92 で「ξ/Δを大きくすると指数が壊れる」ことは分かったが**機構は未解明**のまま残していた(v0.92で自分の「DOS構造を跨ぐため」という説明が誤りと確認済み)。本命題が答えを与える: 乱れた系のスペクトル相関がRMT(剛的)の普遍形を保つのは**Thouless エネルギー E_Th** まで。ξ がそれを超えると測っているものが剛的相関でなくなり、実効指数が無相関(Poisson)側へドリフトする。`g = E_Th/Δ` は**無次元コンダクタンス**なので **有効なξ窓は`[Δ, g·Δ]`——その幅(Δ単位)がg**。**証拠**: (1)金属(W=6)でLを増やすとズレが減る(`L=6: +0.488±0.027` → `L=8: +0.376±0.044`)——3D金属は`g∝L`で窓が広がるという予言通りの向き;(2)**絶縁体(W=30)ではLによらずズレ≈0**(−0.040/+0.121/−0.041)——局在相はPoissonで**準位が全スケールで無相関**、破れる剛性が最初から無い;(3)乱れ走査は非単調(W=4:+0.093→8:+0.318→12:+0.172)だが枠組みと整合(ズレには「剛的plateau」と「小さいg」の**両方**が要る)。**正直な限界**: E_Thを独立に測って**定量比較はしていない**(状況証拠)。効果は本物だが控えめで、**L=6→7の一歩はノイズに埋もれて分解できない**(L=6→8の差0.112がばらつきの約2.5〜4倍)。3D厳密対角化(L≤8)の制約下の結論で、転送行列による直接のE_Th測定が次の一手 |
