...
| `transport.py` | 電子輸送(電気/熱伝導・比熱)。**輸送窓`-∂f/∂E`=kappalogicのNOT検出器**(フェルミ準位まわり幅kTの窓)。窓のモーメントからWiedemann-Franz則(ローレンツ数2.44e-8)・ゾンマーフェルト電子比熱(γ∝g(μ))を再現 |
...
| `examples/` | 実行できるデモ15本(命題32の順序依存発火、v0.76のSP2線形スケーリングDFT、v0.77の量子測定、v0.78のBCS超伝導、v0.80の電子輸送、v0.81の磁性、v0.82の分岐分類、v0.83の色と乱れ、v0.84のAnderson転移、v0.85の乱れた物質デモを含む)+ 外付け実験3件(`kappa_cpu/`, `gauge_adder_chain/`, `cuda/`。下記「関連実験」参照) |
