...
| `disorder.py` | 乱れとAnderson局在(**普遍性クラスへの足がかり**)。IPRから局在長を測り、1Dバンド中心の普遍スケーリング**ζ ∝ W^(−2)**を検証(実測 −1.84〜−1.92、最小Wは有限サイズ効果で下振れ)。**正直な否定的事実**: 1Dでは任意のW>0で全状態が局在し金属-絶縁体転移は無い(命題33と同種)。真のAnderson転移(非平均場、ν≈1.57)は3D——平均場を超える宿題 |
...
| `examples/` | 実行できるデモ13本(命題32の順序依存発火、v0.76のSP2線形スケーリングDFT、v0.77の量子測定、v0.78のBCS超伝導、v0.80の電子輸送、v0.81の磁性、v0.82の分岐分類、v0.83の色と乱れデモを含む)+ 外付け実験3件(`kappa_cpu/`, `gauge_adder_chain/`, `cuda/`。下記「関連実験」参照) |
